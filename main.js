...
function downFlatTone(tone) {
  ...
  return (
    flatMap[toneKey] + (toneKey === "Cb" ? Number(toneOctave) - 1 : toneOctave)
  );
}

const VOLUME_CURVE = [1.0, 0.0]; // 0.61, 0.37, 0.22, 0.14, 0.08, 0.05, 0.0]
adjustedVolumeCurve = [1.0, 0]; // 0.61, 0.37, 0.22, 0.14, 0.08, 0.05, 0.0];
...
playTone = (frequency, type, duration) => {
  ...
  for (let i = 0; i < adjustedVolumeCurve.length; i++) {
    adjustedVolumeCurve[i] =
      (VOLUME_CURVE[i] * document.getElementById("volumeSlider").value) / 100;
  }

 ...
 */
const isFlatTone = tone => /\wb\d/.test(tone)
...
adjustedVolumeCurve = [1.0, 0.61, 0.37, 0.22, 0.14, 0.08, 0.05, 0.0]
...
playTone = (frequency, type, duration) => {
  ...
  o.start(0)
  for (let i = 0; i < adjustedVolumeCurve.length; i++) {
      adjustedVolumeCurve[i] = VOLUME_CURVE[i] * document.getElementById('volumeSlider').value / 100
  }
  // g.gain.exponentialRampToValueAtTime(0.0001,context.currentTime + duration);
  g.gain.setValueCurveAtTime(adjustedVolumeCurve, context.currentTime, duration)
}
...
function playSound (waveType, startFreq, endTime) {
  if (soundObj[arguments[0]] && arguments.length === 1) {
    ...
    gainNode.gain.setValueCurveAtTime(adjustedVolumeCurve, context.currentTime, 2.0)
